...
\begin{corollary}[Euler, Fermat, Wilson]\label{groups:corollary-euler-fermat}
Let $n \geq 2$, and write $a \equiv b \pmod n$ if $n \mid a - b$. Let $U_n$ be the set of residue classes modulo $n$ of the integers $a$ with $\gcd(a, n) = 1$, and $\varphi(n) = |U_n|$ the number of
$0 \leq a < n$ with $\gcd(a, n) = 1$.
\begin{enumerate}
\item Multiplication of representatives makes $U_n$ an abelian group, with identity the class of $1$.
\item (Euler) $a^{\varphi(n)} \equiv 1 \pmod n$ whenever $\gcd(a, n) = 1$.
\item (Fermat) If $p$ is prime and $p \nmid a$, then $a^{p-1} \equiv 1 \pmod p$; and $a^p \equiv a \pmod p$ for every integer $a$.
\item (Wilson) If $p$ is prime, then $(p - 1)! \equiv -1 \pmod p$.
\end{enumerate}
\end{corollary}

\begin{proof}
(1) If $a \equiv a'$ and $b \equiv b'$, then $ab - a'b' = a(b - b') + b'(a - a')$ is divisible by $n$, so the product of classes is well defined; it is associative and commutative because multiplication of
integers is. If $\gcd(a, n) = \gcd(b, n) = 1$ then $\gcd(ab, n) = 1$: a prime dividing $n$ and $ab$ divides $a$ or $b$ (Lemma~\ref{number-systems:lemma-euclid}). By B\'ezout
(Theorem~\ref{number-systems:theorem-bezout}), $\gcd(a, n) = 1$ gives $ax + ny = 1$, so the class of $x$, which is again coprime to $n$, is an inverse of the class of $a$.

(2) By Theorem~\ref{groups:theorem-lagrange} applied to the group $U_n$ of order $\varphi(n)$.

(3) For $n = p$ prime, every $a$ with $0 < a < p$ is coprime to $p$, so $\varphi(p) = p - 1$ and (2) applies. Multiplying by $a$ gives $a^p \equiv a$, which also holds trivially when $p \mid a$.

(4) An element $x$ of $U_p$ is its own inverse exactly when $p \mid x^2 - 1 = (x - 1)(x + 1)$, that is, by Lemma~\ref{number-systems:lemma-euclid}, when $x \equiv \pm1$. The remaining elements of $U_p$ fall into pairs
$\{x, x^{-1}\}$ with product $1$. So the product of all elements of $U_p$, which is the class of $(p - 1)!$, is the class of $1 \cdot (-1) = -1$ (for $p = 2$, $1 \equiv -1$ and the statement is clear).
\end{proof}

\begin{example}\label{groups:example-euler-fermat}
$\varphi(12) = 4$, with $U_{12} = \{1, 5, 7, 11\}$, and every element squares to $1$; so $U_{12}$ is not cyclic, although $U_{12}$ has order $4$ and $U_5 = \{1, 2, 3, 4\}$, also of order $4$, is cyclic,
generated by $2$ ($2^2 = 4$, $2^3 = 8 \equiv 3$, $2^4 = 16 \equiv 1$). Fermat's little theorem gives, for instance, $2^{100} = (2^{10})^{10} \equiv 1 \pmod{11}$. Its converse fails: $561 = 3 \cdot 11 \cdot 17$ is not prime, yet
$a^{560} \equiv 1 \pmod{561}$ for all $a$ coprime to $561$, because $2$, $10$ and $16$ divide $560$, so Fermat's theorem gives the congruence modulo $3$, $11$ and $17$, hence modulo their product, the three primes being distinct
(Theorem~\ref{number-systems:theorem-fundamental-arithmetic}). Such numbers are called \emph{Carmichael numbers}.
\end{example}
