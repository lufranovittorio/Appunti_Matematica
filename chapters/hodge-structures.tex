...
Theorem~\ref{hodge-structures:theorem-weight-one}). So $\End_{\mathrm{HS}}(H) = \{\alpha \in \CC : \alpha(\QQ + \QQ\tau) \subseteq \QQ + \QQ\tau\}$. For such $\alpha$ write $\alpha = a + b\tau$ and
$\alpha\tau = c + d\tau$ with $a, b, c, d \in \QQ$. Then $b\tau^2 + (a - d)\tau - c = 0$. If $b \neq 0$, $\tau$ is a root of a rational quadratic polynomial, and since $\tau \notin \RR$ the field $\QQ(\tau)$ is
imaginary quadratic and contains $\alpha$. If $b = 0$, then $\alpha = a \in \QQ$. Conversely, if $\tau$ is imaginary quadratic, $\QQ(\tau) = \QQ + \QQ\tau$ is a field and every $\alpha \in \QQ(\tau)$ is an
endomorphism. Hence
\[
