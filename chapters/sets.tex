...
\begin{remark}\label{sets:remark-zero-natural}
In this construction $0$ is a natural number, and there is a good reason for it.
The construction needs a first set to start from, and the only set that the
axioms provide without any choices is $\emptyset$. With $0 = \emptyset$, each
natural number is the set of its predecessors,
$n = \{0, 1, \ldots, n - 1\}$, so $n$ is a set with exactly $n$ elements: $0$
counts the empty set, and ``$X$ has $n$ elements'' will mean that $X$ is in
bijection with $n$ (Definition~\ref{sets:definition-finite}). Moreover $0$ is the
neutral element of addition (Example~\ref{sets:example-arithmetic}), so with
$0 \in \NN$ the natural numbers form a monoid under addition, and every natural
number is a sum of ones, the empty sum being $0$. For these reasons
$\NN = \{0, 1, 2, \ldots\}$ throughout the project. Many texts, especially in
analysis and number theory, let $\NN$ start at $1$; when we need the positive
integers we write $\NN_{>0} = \{1, 2, 3, \ldots\}$, so a sequence indexed by
$\NN_{>0}$ starts with its term of index $1$.
\end{remark}
