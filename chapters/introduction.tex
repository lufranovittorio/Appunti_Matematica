...
\begin{remark}\label{introduction:remark-first-reading}
The book also contains material that is not needed on the way to the Hodge conjecture, and is there because it belongs to the same circle of ideas or because it is
beautiful. Chapters and sections of this kind say so at their beginning, in a paragraph ``On a first reading''. For instance, Chapter~\ref{model-theory} is not used by any
other chapter, Chapter~\ref{logic} is needed only for the foundations of set theory, and most of Chapter~\ref{riemannian-geometry} (geodesics beyond the basics, surfaces,
minimal surfaces, space forms) develops geometry for its own sake. The \textbf{Prerequisites} line of each chapter lists the chapters it actually depends on.
\end{remark}
